...
int main(int argc, char *argv[]){
	...
	while(1){
		...
		if(t.f1==16){
			fprintf(file,"<h>Welcome to System Programming Http</h><br>");
		}
		else if(t.f1==24){
			fprintf(file,"<h>System Programming Http</h>,br>");
		}

			
		dirp=opendir(".");
		sprintf(response_message,"<h><b>directory path : %s\n</b></h></br>",getcwd(pth,1024)); // print directory path on header area with bold letters 
		Start(rlist);
		while((dir=readdir(dirp))!=NULL){
			if(t.f2 == 1)
		...
		Statprint(rlist, t,response_message);
